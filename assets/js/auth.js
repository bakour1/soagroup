...
(function(){
  ...
  function sha1(s){
    function rhex(n){var s="", j=0; for(; j<4; j++) s += ("0" + ((n >> (j*8+4)) & 0x0f).toString(16)).slice(-2); return s; }
    ...
    var x = str2blks_SHA1(s), w = new Array(80), a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, e = -1009589776;
    for (var i = 0; i < x.length; i += 16) {
      var olda = a, oldb = b, oldc = c, oldd = d, olde = e;
      for (var j = 0; j < 80; j++) {
        w[j] = (j < 16) ? x[i + j] : (((w[j-3]^w[j-8]^w[j-14]^w[j-16])<<1) | ((w[j-3]^w[j-8]^w[j-14]^w[j-16])>>>31));
        var t = (((a<<5) | (a>>>27)) + e + w[j] + (j<20 ? ((b&c)|((~b)&d)) + 1518500249 : j<40 ? (b^c^d) + 1859775393 : j<60 ? ((b&c)|(b&d)|(c&d)) - 1894007588 : (b^c^d) - 899497514));
  ...
  function readSession(){ try { return JSON.parse(localStorage.getItem(KEY) || "null"); } catch(e){ return null; } }
